...
import { initModel } from './GptModel';
import type { IDataAndModel, IModelState } from './GptModel';
import s from './LayerView.module.scss';
import type { IRenderState, IRenderView } from './render/modelRender';
import { fetchFontAtlasData } from './render/fontRender';
import type { IFontAtlasData } from './render/fontRender';
import { Random } from '@llmviz/utils/random';
import { TensorF32 } from '@llmviz/utils/tensor';
import type { ITensorSet } from '@llmviz/utils/tensor';
import { ProgramStateContext, WalkthroughSidebar } from './Sidebar';
import { initProgramState, runProgram } from './Program';
...
import { KeyboardManagerContext, KeyboardOrder, useGlobalKeyboard } from '@llmviz/utils/keyboard';
...
export function LayerView({
    ...
    useEffect(() => {
        if (!canvasRender || externalPhase === undefined) {
            return;
        }
        ...
        if (wt.phase !== externalPhase) {
            wt.phase = externalPhase;
            wt.time = 0;
            changed = true;
        }

        if (wt.running !== externalRunning) {
            wt.running = externalRunning;
            changed = true;
        }
    ...
    }, [canvasRender, externalPhase, externalRunning]);

    let sidebar = showSidebar && canvasRender && <div className={s.sidebar}>
    ...
    let mainView = <div className={s.canvasWrap}>
        ...
        {canvasRender && <ProgramStateContext.Provider value={canvasRender.progState}>
            ...
            {showToolbar && <ModelSelectorToolbar />}
        </ProgramStateContext.Provider>}
    </div>;

    return <div className={[s.view, className].filter(Boolean).join(' ')}>
        {showSidebar ? <Resizer id={"llm-sidebar"} className={"flex-1"} vertical={!layout.isDesktop} defaultFraction={0.4}>
